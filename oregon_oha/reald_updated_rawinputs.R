  ...
  mutate(
    ...
    LatCen =  (hisplog | racsor) & (
      ...
      if_any(
        ...
        ~ grepl('costa', .)  | grepl('hondur', .) | grepl('nicarag', .) |
          grepl('panama', .) | grepl('beliz', .)  | grepl('(other\\s)?central\\samerican(\\soth)?$', .)
      ) | (
        ...
        (grepl('beliz', pobp_str) | grepl('costa', pobp_str) | grepl('hondur', pobp_str) | 
          ...
          (hisp %in% 24) &
          (anc1p %in% c(250, 290, 996:999) | anc2p %in% c(250, 290))
      )
    ),
    ### - Cuban
    ...
    AIANLat = (racaian | racsor) & (
      ...
      if_any(
        ...
        ~ grepl('((central)|(south)|(mexican)|(latin))\\samerican\\sindian', .) |
          grepl('aztec', .) | grepl('inca', .) | grepl('maya', .) | 
          grepl('mixtec', .) | grepl('taino', .) | grepl('tarasc', .)  
      ) |
        # OR
        ...
        (grepl('specified', rac2p_str) & pobp %in% 300:399)
    ),
    ### - American Indian
    ...
    AIANInd = (racaian) & !grepl('^india$', pobp_str) & !(AIANLat | AIANAlask) & (
      ...
      # NOTE: there's a rac3 item that gets flagged here but not in the old script...
        ...
        # (RACED > 800 & (grepl('AIAN', raced.char) | grepl('[Aa]merican\\s[Ii]ndian', raced.char)) & !(BPL %in% c(110:300, 521)))
          (grepl('specified', rac2p_str) & !(pobp_dict %in% c(210, 300:399)))
      ),
    ...
    AfrAm = racblk & (
      ...
      (if_any(c(anc1p_str, anc2p_str), ~ grepl('afr.+amer', .) | grepl('united\\sstates', .) | grepl('black', .)) | pobp < 67)
    ),
    ### - Afro-Caribbean
    # IDs as Black AND
    AfrCarib = racblk & (
      if_any(
        ...
        ~ grepl('puerto', .) | grepl('cuba', .)  | grepl('dominic', .) | grepl('baham', .) |
    ...
    AfrEthiopian = racblk & if_any(c(anc1p_str, anc2p_str, pobp_str, lanp_str), ~ grepl('ethio', .) | grepl('eritr', .) | grepl('amhar', .)),
    ### - Haitian
    ...
    AsnInd = (racasn | racsor | (racaian & grepl('^india$', pobp_str))) & (
      # Ancestry includes Asian Indian, Bengali, East Indies, Punjabi, Karnatakan, Assamese, Gujarati
      if_any(
        c(anc1p_str, anc2p_str),
        ~ grepl('asian\\sindia', .) | grepl('bengal', .) | grepl('east\\sindi[ea]', .) | grepl('punjab', .) |
          grepl('karna', .) | grepl('assam', .) | grepl('gujar', .)
      ) | (
      # OR  born in India and speaking Indian language
        grepl('^india$', pobp_str) & (
          grepl('hindi', lanp_str)| grepl('urdu', lanp_str) | grepl('other\\sindo', lanp_str) | grepl('sanskr', lanp_str) |
          grepl('bengal', lanp_str) | grepl('p[ua]njab', lanp_str) | grepl('marath', lanp_str) | grepl('gujara', lanp_str) |
          grepl('bihari', lanp_str) | grepl('rajasth', lanp_str) | grepl('oriya', lanp_str) | grepl('assam', lanp_str) |
          grepl('kashmi', lanp_str) | grepl('kannad', lanp_str) | grepl('dravid', lanp_str) | grepl('teleg', lanp_str) |
          grepl('malayal', lanp_str)
        ) 
      ) |
      # OR specifies "Asian indian" in race
        if_any(c(rac2p_str, rac3p_str), ~ grepl('asian\\sindia', .))
    ...
    AsnCambod = (racasn | rac1p %in% 8) & (
      ...
      if_any(c(anc1p_str, anc2p_str, rac2p_str, lanp), ~ grepl('cambod', .) | grepl('khmer', .)) | (
    ...
    AsnMyan = (racasn | rac1p %in% 8) & (
      ...
      if_any(
        c(anc1p_str, anc2p_str, rac2p_str, rac3p_str, lanp),
    ...
    AsnFilipino = (racasn | rac1p %in% 8) & (
      ...
      if_any(c(anc1p_str, anc2p_str, rac2p_str, rac3p_str), ~ grepl('filip', .)) | (
        ...
        grepl('([^(\\/or)]\\s|^)other\\sasian', rac3p_str) & (
          ...
          (grepl('tagal', lanp_str) | grepl('filip', lanp_str) | grepl('[il]loc', lanp_str) | grepl('[cs]ebua', lanp_str)) | (
    ...
    AsnHmong = (racasn | rac1p %in% 8) & (
      ...
      if_any(
        ...
        ~ grepl('hmong', .) | grepl('iu\\smien', .) | grepl('miao', .)
      )
    ),
    ### - Indonesian
    ...
    AsnKorean = (racasn | rac1p %in% 8) & (
      ...
      if_any(c(anc1p_str, anc2p_str, rac2p_str, rac3p_str), ~ grepl('kor', .)) |
    ...
    AsnViet = (racasn | rac1p %in% 8) & (
      # ancestry or race includes vietnamese
      if_any(c(anc1p_str, anc2p_str, rac2p_str, rac3p_str), ~ grepl('([^(\\/or)]\\s|^)viet', .)) | (
    ...
    ### - Native Hawai'ian
    # NOTE: in next version just use racnh...
    NHPIHawaii = (racnh | rac1p %in% 8) & (
      # Language or ancestry includes Hawaiian
      if_any(c(anc1p_str, anc2p_str, lanp_str), ~ grepl('hawai', .) & !grepl("^other", .)) |
        # or race includes native hawaiian (but not the uninformative "and/or" NH) 
        if_any(c(rac2p_str, rac3p_str), ~ grepl('([^(\\/or)]\\s|^)native\\shawaiian', .))
    ),
    ### - Marshallese
    # IDs as Pacific Islander and
    NHPIMarshall = (racpi | rac1p %in% 8) & (
    ...
    NHPIOther = (racpi | (rac1p %in% 8 & (lanp %in% 3220:3600 | pobp %in% c(508:512, 523:527))) ) &
    ...
    MENAEgypt = (
      ...
      if_any(c(anc1p_str, anc2p_str), ~ grepl('egypt', .)) | 
        ...
        (if_any(c(anc1p_str, anc2p_str), ~ grepl('arab$', .) | grepl('mideast', .)) & grepl('egypt', pobp_str))
    ),
    ### - Iraqi
    MENAIraq = (
      ...
      if_any(c(anc1p_str, anc2p_str), ~ grepl('iraq', .)) |
        ...
        (
          if_any(c(anc1p_str, anc2p_str), ~ grepl('arab$', .) | grepl('mideast', .) | grepl('assyri', .) | grepl('kurd', .)) &
            grepl('iraq', pobp_str)
        )
    ),
    ### - Iranian
    ...
    MENAIsr = if_any(c(anc1p_str, anc2p_str), ~ grepl('israel', .)) | 
      ...
      (grepl('israel', pobp_dict) & (grepl('hebrew', lanp_str) | grepl('yidd', lanp_str))),
    ### - Lebanese
    ...
    MENALeban = if_any(c(anc1p_str, anc2p_str), ~ grepl('leban', .)) | (
      ...
      grepl('leban', pobp_str) & (
        ...
        if_any(c(anc1p_str, anc2p_str), ~ grepl('arab$', .) | grepl('mideast', .)) |
          grepl('arabic', lanp_str) | grepl('aramaic', lanp_str)
      )
    ),
    ### - Palestinian
    ...
    MENASyr = if_any(c(anc1p_str, anc2p_str), ~ grepl('^syria', .)) | (
      ...
      grepl('syria', pobp_str) & (
        ...
        if_any(c(anc1p_str, anc2p_str), ~ grepl('arab$', .) | grepl('mideast', .) | grepl('assyr', .) | grepl('kurd', .)) |
          grepl('arabic', lanp_str) | grepl('aramaic', lanp_str)
      )
    ),
    ### - Turkish
    ...
    WhtEng = (racwht | (anc1p < 210 & (anc2p < 210 | anc2p %in% 999) & rac1p %in% 8)) & (
      ...
      if_any(c(anc1p_str, anc2p_str), ~ grepl('^engl', .)) | (
        ...
        grepl('engl', pobp_str) & is.na(lanp) & (
          grepl('(^[nw].+|^)euro', anc1p_str) | anc1p %in% c(996, 999)
        )
      )
    ),
    ### - German
    ...
    WhtIre = (racwht | (anc1p < 210 & (anc2p < 210 | anc2p %in% 999) & rac1p %in% 8)) & (
      ...
      if_any(c(anc1p_str, anc2p_str), ~ grepl('^iri', .) | grepl('^celt', .)) | (
        ...
        grepl('^ire', pobp_str) & (is.na(lanp) | grepl('^iri', lanp_str)) & (
          grepl('^[nw].+euro', anc1p_str) | grepl('^euro', anc1p_str) | anc1p %in% c(996, 999)
        )
      )
    ),
    ### - Italian
    ...
    WhtItal = (racwht | (anc1p < 210 & (anc2p < 210 | anc2p %in% 999) & rac1p %in% 8)) & (
      ...
      if_any(c(anc1p_str, anc2p_str), ~ grepl('ital', .) | grepl('sicil', .)) | (
        ...
        grepl('ital', pobp_str) & (grepl('(^s.+|^)euro', anc1p_str) | (anc1p %in% c(996, 999)))
      )
    ),
    ### - Polish
    # White, and 
    WhtPol = (racwht | (anc1p < 210 & (anc2p < 210 | anc2p %in% 999) & rac1p %in% 8)) & (
      ...
      if_any(c(anc1p_str, anc2p_str), ~ grepl('polish', .)) | (
        ...
        if_any(c(lanp_str, pobp_str), ~ grepl('pol[ia]', .)) & 
          (grepl('(^e.+|^)euro', anc1p_str) | (anc1p %in% c(996, 999)))
      )
    ),
    ### - Romanian
    # White, and 
      ...
      if_any(c(anc1p_str, anc2p_str), ~ grepl('^russ', .)) | (
        ...
        grepl('^russ', lanp_str) & (
          ...
          if_any(c(anc1p_str, anc2p_str), ~ grepl('^turk.+stan', .)) | (
            ...
            (
              grepl('^russ', pobp_str) | grepl('ussr', pobp_str) | grepl('georgia$', pobp_str) |
    ...
    WhtSco = (racwht | (anc1p < 210 & (anc2p < 210 | anc2p %in% 999) & rac1p %in% 8)) & (
      ...
      if_any(c(anc1p_str, anc2p_str), ~ grepl('^scot', .)) | (
        grepl('^scot', pobp_str) & is.na(lanp) & ((grepl('(^[nw].+|^)euro', anc1p_str) | (anc1p %in% c(996, 999))))
      )
    ),
    ### - Ukrainian
    # White, and
    ...
    WhtSlav = (racwht | (anc1p < 210 & (anc2p < 210 | anc2p %in% 999) & rac1p %in% 8)) & (
      if_any(
        ...
        # Serbian, Macedonian, Slovakian, Slovenian, Yugoslavian, Slav
        c(anc1p_str, anc2p_str),
        ~ grepl('bulga', .) | grepl('belor', .) | grepl('czech', .) | grepl('croat', .) | grepl('^bohe', .) |
          grepl('serb', .) | grepl('maced', .) | grepl('slov[ae]', .) | grepl('yugos', .) | grepl('slav', .)
      ) | (
        grepl('czech', lanp_str) | grepl('slov[ea]', lanp_str) | grepl('croat', lanp_str) |
          grepl('serb', lanp_str) | grepl('bosn', lanp_str)
      ) |
        (
          # or, born in Bulgaria, Czechoslovakia, Yugoslavia and un-listed/non-descript ancestry
          (
            grepl('bulga', pobp_str) | grepl('czech', pobp_str) | grepl('sl[oa]v', pobp_str)
          ) & (
            ((grepl('(^[es].+|^)euro', anc1p_str) | (anc1p %in% c(996, 999))))
          )
        )
    ),
    ### - White, other
...
#   apply(2, sum) |>
#   (\(vec) data.frame(reald = names(vec), y = vec))()

new_counts = pums_out |>
  select(starts_with(c('AIAN', 'Asn', 'Afr', 'Lat', 'MENA', 'NHPI', 'Wht', 'Other'))) |>
  apply(2, sum) |>
  (\(vec) data.frame(reald = names(vec), x = vec))()

compare_counts = merge(new_counts, orig_counts, all = TRUE)

compare_counts |>
  filter(x != y) |>
  arrange(desc(abs(x - y))) |>
  mutate(diff = x - y)

pums_compare = merge(
  pums_out |> select(serialno, sporder, NHPICham),
  orig_reald |> filter(NHPICham) |> select(CBSERIAL, PERNUM) |> mutate(in.y = TRUE),
  by.x = c('serialno', 'sporder'), by.y = c('CBSERIAL', 'PERNUM'),
  all = TRUE
) |>
  mutate(in.y = ifelse(is.na(in.y), FALSE, in.y)) |>
  filter(NHPICham | in.y)

pums_compare |>
  merge(pums_out |> select(serialno, sporder, racpi, racnh, ends_with('str'))) |> select(-hisp_str) |>
  filter(NHPICham + in.y < 2) # |> select(anc1p_str, anc2p_str, lanp_str, AIANInd, in.y)
