...
# Get PUMS ddi
pums.raw.ddi = read_ipums_ddi('multnomah/01_raw_data/usa_00056.xml')

# Read in PUMS and subset to MultCo
pums.raw = read_ipums_micro(pums.raw.ddi)

# Read in tabular data
acs.tab.raw1 = read_nhgis('multnomah/01_raw_data/nhgis0036_csv.zip', file_select = 1) %>%
  filter(grepl('Multno', COUNTY))
acs.tab.raw2 = read_nhgis('multnomah/01_raw_data/nhgis0036_csv.zip', file_select = 2) %>%
  filter(grepl('Multno', COUNTY))
...
y.tab = acs.tab %>%
  ...
  mutate(var_label = ifelse(is.rac, univ, var_label)) %>%
  # Arrange rows:
  arrange(
...
x.mat = x.tab %>%
  ...
  complete(
    sex, nesting(age.fine, age.vets, age.disb), ancestry, hispanic,
    mar, veteran, has.diff, nesting(family.hous, lives.alone)
  ) %>%
  arrange(
    sex, age.fine, age.vets, age.disb, ancestry, hispanic, 
  ...
  mutate(
    ...
    in.gq = as.numeric(GQ %in% 3:5 & PERNUM < 2),
    ### Race columns
    rac.amind = RACAMIND,
    rac.asian = RACASIAN,
    rac.black = RACBLK,
    ...
    ##########################
    # Here:
    # I'm pretty sure `hou.alo.one` WAS overcounting here
    # by recycling total.hou, which is a 1 for ANY household type
    # but we only want this variable to be 1 for SOME household types (otherwise would overcount)
    # I also went back and monkeyed with the flag for family/nonfamily housing
    hou.alo.one = total.hou, # as.numeric(family.hous %in% 'nonfam.hou' & PERNUM < 2),
    ### Grandparents
    gpa.cols = grandparent,
    gpa.ones = total.hou # * as.numeric(family.hous %in% 'fam.hou'),
  ) %>%
  # Pivot out hispnic total
