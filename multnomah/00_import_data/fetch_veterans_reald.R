...
acs_extract = define_extract_micro(
  collection = 'usa',
  description = 'PUMS for age+disability+ancestry, Mult Co. draw 5 with grandparent info',
  samples = 'us2023c',
  variables = list(
    var_spec('STATEFIP', case_selections = '41'),
    var_spec('COUNTYICP', case_selections = '0510'),
...
acs_extract %>%
  ...
  wait_for_extract() %>%
  download_extract(download_dir = 'multnomah/01_raw_data/')
# Downloaded as extract # 63

acs_extract = define_extract_micro(
  collection = 'usa',
  description = 'PUMS for age+disability+ancestry, Mult Co. draw 6 with GCRESP and GCHOUSE',
  samples = 'us2023c',
  variables = list(
    var_spec('STATEFIP', case_selections = '41'),
    var_spec('COUNTYICP', case_selections = '0510'),
    ...
    'ANCESTR1', 'ANCESTR2',
    'VETSTAT', 'OCC', 'GQ',
    'DIFFCARE', 'DIFFMOB', 'DIFFPHYS', 'DIFFREM', 'DIFFEYE', 'DIFFHEAR',
    'MARST', 'RELATE', 'GCHOUSE', 'GCRESPON',
    # Housing cost variables:
    'OWNERSHP', 'OWNCOST', 'RENTGRS', 'HHINCOME'
  )
)
...
tab_extract = define_extract_nhgis(
  description = 'tract-level veteterans+related for controls, draw 4 with grandparentage',
  datasets = map2(
    .x = list('2019_2023_ACS5a', '2019_2023_ACS5b'),
    .y = list(
      c(
